...
device_map and attn_implementation are omitted (GPU not present at build time).
"""
import sys
import torch
...
def download(name, model_id):
    ...
    try:
        # dtype=bfloat16 matches what the handler uses at runtime,
        # so the cached files don't need dtype conversion on load.
